...
from pyspark.sql.functions import (
    col, to_date, expr, lag, log, avg, stddev,
    min as smin, max as smax, count, when, lit,
    udf, datediff, sum as ssum, isnan
)
...
rets = pxdf.withColumn(
    "prev",
    lag("close").over(w)
)
rets.show(5)
rets = rets.withColumn(
    "ret",
    log(col("close") / col("prev"))
).dropna(subset=["ret"])
...
rv = rets.withColumn("vol20",stddev("ret").over(w20) * math.sqrt(252)).withColumn("vol60",stddev("ret").over(w60) * math.sqrt(252))
...
if "last_trade" in optdf.columns:
    optdf = optdf.withColumn(
        "last_trade",
        to_date(col("last_trade"))
    )
...
type_sum = optdf.groupBy("type").agg(
...
po = optdf.withColumn("s", lit(float(spot))) \
...
po = po.withColumn(
    "be",
    when(col("type") == "call", col("k") + col("mid"))
    .when(col("type") == "put", col("k") - col("mid"))
)
...
po = po.withColumn(
    "be_dist",
    when(col("type") == "call", (col("be") - col("s")) / col("s"))
    .when(col("type") == "put", (col("s") - col("be")) / col("s"))
)
...
po = po.withColumn(
    "up10_ret",
    when(col("mid") > 0, (col("bs_up10") - col("mid")) / col("mid"))
).withColumn(
    "dn10_ret",
    when(col("mid") > 0, (col("bs_dn10") - col("mid")) / col("mid"))
)
...
po = po.withColumn(
    "spr",
    when(col("mid") > 0, (col("ask") - col("bid")) / col("mid"))
).withColumn(
    "prem",
    when(col("bs") > 0, (col("mid") - col("bs")) / col("bs"))
).withColumn(
    "iv_gap",
    col("iv") - col("hist_vol")
)
...
po = po.withColumn(
    "f_spread",
    when(col("spr") > 0.30, 1).otherwise(0)
)
...
po = po.withColumn(
    "f_vol",
    when(col("opt_vol") < 10, 1).otherwise(0)
)
...
po = po.withColumn(
    "f_be",
    when(col("be_dist") > 0.15, 1).otherwise(0)
)
...
po = po.withColumn(
    "f_prob",
    when(col("itm_prob") < 0.25, 1).otherwise(0)
)
...
po = po.withColumn(
    "f_prem",
    when(col("prem") > 0.50, 1).otherwise(0)
)
...
po = po.withColumn(
    "f_iv",
    when(col("iv") > col("hist_vol") * 1.30, 1).otherwise(0)
)
...
po = po.withColumn(
    "score",
    col("f_spread") + col("f_oi") + col("f_vol") +
    col("f_be") + col("f_prob") + col("f_prem") + col("f_iv")
)
...
po.select(
    "name", "type", "k", "mid", "bs", "iv", "spr",
    "oi", "opt_vol", "be_dist", "itm_prob", "score", "risk"
).orderBy(
    col("score").desc(),
    col("spr").desc()
...
risk_sum = po.groupBy("type", "risk").agg(
    count("*").alias("n")
).orderBy("type", "risk")
risk_sum.show()
...
# %%
#Daily Volume
dvol = vdf.groupBy("dt", "ticker").agg(
    ssum("vol").alias("tot_vol")
).orderBy("dt")
dvol.show(10)
...
dvol = dvol.withColumn(
    "weird_vol",
    when(col("tot_vol") > 2 * col("ma20"), 1).otherwise(0)
)
dvol.orderBy(col("tot_vol").desc()).show(10)

# %%
#Latest Volume
latest_vol = dvol.orderBy(col("dt").desc()).limit(1).select(
    col("dt").alias("vol_dt"),
    col("tot_vol"),
    col("ma20"),
    col("weird_vol")
...
po2.select(
    "name", "type", "k", "mid", "score", "risk",
    "vol_dt", "tot_vol", "ma20", "weird_vol"
...
p_opt = po.select(
    "type", "k", "mid", "bs", "iv", "risk", "spr",
    "score", "be_dist", "itm_prob", "up10_ret", "dn10_ret"
).toPandas()
